...
#   PROFESSIONAL FINTECH EDITION  |  Fixed Navigation
...
# ─────────────────────────────────────────────────────────────
# PAGE CONFIG  — must be FIRST streamlit call, nothing before it
# ─────────────────────────────────────────────────────────────
...
.nav-btn:hover  { background: var(--teal-glow); border-color: var(--border-active); color: var(--teal); }
.nav-btn.active { background: var(--teal-glow); border-color: var(--border-active); color: var(--teal); font-weight: 600; }
...
.card:hover { border-color: var(--border-active); box-shadow: 0 0 40px rgba(20,184,166,0.06); }
...
.kpi:hover { border-color: var(--border-active); transform: translateY(-2px); box-shadow: 0 8px 30px rgba(0,0,0,.3); }
.kpi-val   { font-size: 1.8rem; font-weight: 800; color: var(--text-primary); line-height: 1; }
.kpi-lbl   { font-size: 0.72rem; color: var(--text-muted); text-transform: uppercase; letter-spacing: .08em; margin-top: 6px; }
.kpi-icon  { font-size: 1.4rem; margin-bottom: 10px; display: block; }

/* ── Badges ── */
.badge      { border-radius: 12px; padding: 18px 24px; font-size: 1.1rem; font-weight: 600; text-align: center; }
.badge-low  { background: rgba(20,184,166,.08);  border: 1px solid rgba(20,184,166,.3);  color: #2dd4bf; }
.badge-med  { background: rgba(245,158,11,.08);  border: 1px solid rgba(245,158,11,.3);  color: #fbbf24; }
.badge-high { background: rgba(244,63,94,.08);   border: 1px solid rgba(244,63,94,.3);   color: #fb7185; }

/* ── Section label ── */
.section-label  { font-size: .7rem; font-weight: 600; text-transform: uppercase; letter-spacing: .1em; color: var(--teal); margin-bottom: 8px; }
.page-title     { font-size: 1.75rem; font-weight: 700; color: var(--text-primary); letter-spacing: -.5px; margin-bottom: 4px; }
.page-subtitle  { font-size: .9rem; color: var(--text-muted); margin-bottom: 28px; }
.accent-line    { width: 40px; height: 3px; background: linear-gradient(90deg,var(--teal),var(--indigo)); border-radius: 2px; margin-bottom: 20px; }
...
def apply_theme(fig, title="", height=380):
    fig.update_layout(
        ...
        title         = dict(text=title, font=dict(size=14, color="#e2e8f0", family="Inter"), x=.02, y=.97),
        font          = dict(color=TEXT_C, family="Inter", size=11),
        legend        = dict(bgcolor="rgba(0,0,0,0)", font=dict(color=TEXT_C, size=11),
                             bordercolor="rgba(255,255,255,0.05)", borderwidth=1),
        hoverlabel    = dict(bgcolor="#1e293b", font_size=12, font_color="#f1f5f9", bordercolor="#334155"),
    )
    fig.update_xaxes(gridcolor=GRID, zerolinecolor=GRID, tickfont=dict(color=TEXT_C), title_font=dict(color=TEXT_C))
    fig.update_yaxes(gridcolor=GRID, zerolinecolor=GRID, tickfont=dict(color=TEXT_C), title_font=dict(color=TEXT_C))
    return fig


# ─────────────────────────────────────────────────────────────
# SESSION STATE — single source of truth for active page
# ─────────────────────────────────────────────────────────────

PAGES = ["Overview", "Credit Scorer", "Fraud Detector", "Portfolio Analytics", "Batch Scoring"]
PAGE_ICONS = ["◈", "💳", "🔍", "📊", "📁"]
...
# ─────────────────────────────────────────────────────────────
# SIDEBAR — navigation driven entirely by session_state
# ─────────────────────────────────────────────────────────────

with st.sidebar:
    st.markdown("""
    <div style='padding:24px 8px 16px;border-bottom:1px solid rgba(148,163,184,0.08);margin-bottom:16px;'>
        <div style='font-size:.7rem;color:#14b8a6;font-weight:600;letter-spacing:.15em;text-transform:uppercase;margin-bottom:6px;'>
            Final Year Project
        </div>
        <div style='font-size:1.3rem;font-weight:800;color:#f1f5f9;letter-spacing:-.5px;'>RiskIQ</div>
        <div style='font-size:.78rem;color:#475569;margin-top:2px;'>Banking Intelligence Platform</div>
    </div>
    """, unsafe_allow_html=True)

    # One button per page — highlighted when active
    for icon, label in zip(PAGE_ICONS, PAGES):
        is_active = st.session_state.active_page == label
        btn_style = "background:rgba(20,184,166,0.12);border:1px solid rgba(20,184,166,0.35);color:#14b8a6;font-weight:600;" if is_active else ""
        # Streamlit button — on_click updates session_state
        if st.button(
            f"{icon}  {label}",
            key=f"nav_{label}",
    ...
    st.markdown("""
    <div style='margin-top:24px;padding-top:20px;border-top:1px solid rgba(148,163,184,0.08);'>
        <div style='font-size:.68rem;color:#14b8a6;font-weight:600;letter-spacing:.1em;text-transform:uppercase;margin-bottom:12px;'>
            System Status
        </div>
        <div style='font-size:.8rem;color:#64748b;line-height:2.2;'>
            ...
            <span style='color:#2dd4bf;'>●</span> AutoEncoder &nbsp; AUC 0.7266<br>
            <span style='color:#2dd4bf;'>●</span> SHAP &nbsp; Enabled
        </div>
    </div>
    <div style='margin-top:20px;padding-top:16px;border-top:1px solid rgba(148,163,184,0.08);'>
        <div style='font-size:.68rem;color:#475569;line-height:2;'>
...
# ─────────────────────────────────────────────────────────────
# LOAD MODELS & DATA  (cached — loaded once, reused across pages)
# ─────────────────────────────────────────────────────────────
...
@st.cache_data(show_spinner="Loading LendingClub data…")
def load_lc():
    try:
...
if page == "Overview":
    ...
    <div class='page-subtitle'>AI-driven loan default prediction · anomaly detection · explainable decisions</div>
    ...
    with col1:
        ...
        models_  = ["Logistic Regression","Random Forest","XGBoost (Tuned)"]
        aucs_    = [0.9969, 0.9990, 0.9997]
        f1s_     = [0.98,   0.99,   0.99]
        colors_b = [INDIGO, VIOLET, TEAL]
        fig = go.Figure()
        for i, (m, auc, f1, color) in enumerate(zip(models_, aucs_, f1s_, colors_b)):
            fig.add_trace(go.Bar(
                name=m, x=["ROC-AUC","F1-Score"], y=[auc, f1],
                marker_color=color,
                marker_line=dict(color="rgba(255,255,255,0.05)", width=1),
                text=[f"{auc:.4f}", f"{f1:.2f}"], textposition="outside",
                textfont=dict(color="#e2e8f0", size=11),
                offsetgroup=i, width=0.22,
            ))
    ...
    with col2:
        ...
        if portfolio is not None:
            ...
            fig = go.Figure(go.Pie(
                ...
                marker=dict(colors=[TIER_COLOR.get(t, INDIGO) for t in tc.index],
            ...
            fig.add_annotation(text=f"<b>{len(portfolio):,}</b><br><span style='font-size:10px'>customers</span>",
    ...
    if lc is not None:
        ...
        fig = go.Figure(go.Heatmap(
            ...
            texttemplate="%{text}", textfont=dict(size=9, color="#e2e8f0"),
            hovertemplate="<b>%{x}</b> × <b>%{y}</b><br>r = %{z:.3f}<extra></extra>",
            showscale=True,
            colorbar=dict(tickfont=dict(color=TEXT_C), title=dict(text="r", font=dict(color=TEXT_C)),
                          outlinecolor=GRID, outlinewidth=1, thickness=14, len=0.85)
        ))
    ...
    if lc is not None and "purpose" in lc.columns:
        ...
        with col3:
            ...
            pur = lc.groupby("purpose", observed=True)["target"].mean().sort_values() * 100
            fig = go.Figure(go.Bar(
                x=pur.values, y=pur.index.astype(str), orientation="h",
                marker=dict(color=pur.values, colorscale=SEQ_MULTI, showscale=False),
                text=[f"{v:.1f}%" for v in pur.values], textposition="outside",
                textfont=dict(color="#e2e8f0", size=10),
                hovertemplate="<b>%{y}</b><br>Default: %{x:.1f}%<extra></extra>",
            ))
        ...
        with col4:
            ...
            if "int_rate" in lc.columns:
                sample = lc.sample(min(3000, len(lc)), random_state=42)
                fig = go.Figure()
                for t_val, color, name in [(0, TEAL,"Fully Paid"),(1, ROSE,"Charged Off")]:
                    mask = sample["target"] == t_val
                    fig.add_trace(go.Scatter(
                        x=sample[mask].get("int_rate", pd.Series()),
                        y=sample[mask].get("dti", pd.Series()),
                        mode="markers", name=name,
                        marker=dict(color=color, size=3.5, opacity=0.55, line=dict(width=0)),
...
# ═════════════════════════════════════════════════════════════
    ...
    <div class='page-subtitle'>Input customer financial profile to generate a risk-adjusted credit score</div>
    <div class='accent-line'></div>
    """, unsafe_allow_html=True)

    if not MODELS_OK:
        st.error("Models unavailable. Run day2_model.py to generate model artifacts.")
        st.stop()
    ...
    if st.button("Generate Risk Assessment"):
        ...
        if prob < 0.30:
            badge_cls, badge_txt, tier, t_color = "badge-low",  f"Low Risk — Default Probability: {prob:.1%}",      "LOW",    TEAL
        elif prob < 0.60:
            badge_cls, badge_txt, tier, t_color = "badge-med",  f"Elevated Risk — Default Probability: {prob:.1%}", "MEDIUM", AMBER
        else:
            badge_cls, badge_txt, tier, t_color = "badge-high", f"High Risk — Default Probability: {prob:.1%}",     "HIGH",   ROSE

        r1, r2, r3, r4 = st.columns([3, 1, 1, 1])
        r1.markdown(f'<div class="badge {badge_cls}">{badge_txt}</div>', unsafe_allow_html=True)
        r2.metric("Default Prob", f"{prob:.1%}")
        r3.metric("Risk Tier", tier)
        r4.metric("Decision", "Reject" if prob > 0.5 else "Approve")
        ...
        with cg:
            ...
            fig = go.Figure(go.Indicator(
                ...
                title = {"text": "Default Probability", "font": {"color": TEXT_C, "size": 13}},
                gauge = {
                    "axis"      : {"range": [0,100], "tickcolor": TEXT_C, "tickwidth": 1,
                                   "tickfont": {"color": TEXT_C, "size": 10}},
                    "bar"       : {"color": t_color, "thickness": 0.25},
                    "bgcolor"   : "#111827", "bordercolor": "#1e293b", "borderwidth": 2,
                    "steps"     : [{"range":[0,30], "color":"rgba(20,184,166,0.08)"},
            ...
            fig.update_layout(paper_bgcolor=BG, height=280, margin=dict(l=20,r=20,t=30,b=10),
        ...
        with cr:
            ...
            fig = go.Figure(go.Scatterpolar(
                ...
                fill="toself",
                fillcolor="rgba(244,63,94,0.1)" if prob > 0.5 else "rgba(20,184,166,0.1)",
            ...
            fig.update_layout(
                polar=dict(bgcolor="#111827",
                           radialaxis=dict(visible=True, range=[0,1], gridcolor=GRID,
                                           tickfont=dict(color=TEXT_C, size=9), linecolor=GRID),
                           angularaxis=dict(gridcolor=GRID, tickfont=dict(color=TEXT_C, size=10), linecolor=GRID)),
                paper_bgcolor=BG, height=280, margin=dict(l=50,r=50,t=30,b=10),
                showlegend=False, font=dict(color=TEXT_C, family="Inter"),
            )
            st.plotly_chart(fig, use_container_width=True)
            st.markdown("</div>", unsafe_allow_html=True)
        ...
        st.caption("Features in red increase default risk · Features in blue reduce it")
        shap_vals = explainer.shap_values(row_df)
        plt.style.use("dark_background")
        fig_s, ax = plt.subplots(figsize=(11, 5))
        fig_s.patch.set_facecolor("#111827")
        ax.set_facecolor("#111827")
        shap.waterfall_plot(
            shap.Explanation(values=shap_vals[0], base_values=explainer.expected_value,
                             data=row_df.iloc[0], feature_names=feat_cols),
            show=False)
...
# ═════════════════════════════════════════════════════════════
    ...
    <div class='page-subtitle'>Isolation Forest model identifies statistically anomalous financial behaviour</div>
    <div class='accent-line'></div>
    """, unsafe_allow_html=True)
    ...
    if st.button("Run Anomaly Detection"):
        ...
        st.markdown(f'<div class="badge {b_cls}" style="margin-bottom:20px">{b_txt}</div>', unsafe_allow_html=True)
        ...
        with cg:
            ...
            fig = go.Figure(go.Indicator(
                ...
                title = {"text": "Fraud Probability", "font": {"color": TEXT_C, "size": 13}},
                gauge = {
                    "axis"    : {"range": [0,100], "tickcolor": TEXT_C, "tickwidth": 1,
                                 "tickfont": {"color": TEXT_C, "size": 10}},
            ...
            fig.update_layout(paper_bgcolor=BG, height=270, margin=dict(l=20,r=20,t=30,b=10),
                              font=dict(color=TEXT_C, family="Inter"))
            st.plotly_chart(fig, use_container_width=True)
            st.markdown("</div>", unsafe_allow_html=True)
        ...
        with cb:
            ...
            fig = go.Figure(go.Bar(
                ...
                marker=dict(color=scores, colorscale=[[0,TEAL],[0.5,AMBER],[1,ROSE]],
                            showscale=False, line=dict(color="rgba(255,255,255,0.04)", width=1)),
                text=[f"{v:.0f}%" for v in scores], textposition="outside",
                textfont=dict(color="#e2e8f0", size=11),
...
# ═════════════════════════════════════════════════════════════
    ...
    <div class='page-subtitle'>Aggregate risk metrics · customer segmentation · distribution analysis</div>
    <div class='accent-line'></div>
    """, unsafe_allow_html=True)
    ...
    if portfolio is None:
        st.error("Run day3_fraud.py first to generate portfolio data.")
        st.stop()
    ...
    kpi_data = [
        ...
        ("◎", f"{med_r:,}",   "Medium Risk"),
        ("◆", f"{low_r:,}",   "Low Risk"),
        ("◇", f"{avg_c:.1%}", "Avg Credit Score"),
        ("○", f"{avg_f:.1%}", "Avg Fraud Score"),
    ]
    cols = st.columns(6)
    ...
    with col1:
        ...
        fig.add_annotation(text=f"<b>{len(filtered):,}</b>", x=0.5, y=0.5, showarrow=False,
        ...
        for tier, color in TIER_COLOR.items():
            ...
            if mask.sum() > 0:
                ...
                fig.add_trace(go.Violin(
                    y=filtered[mask]["unified_score"], name=tier,
                    box_visible=True, meanline_visible=True,
                    line_color=color, fillcolor=f"rgba({r},{g},{b},0.12)",
                    opacity=0.85, points=False,
                ))
    ...
    if lc is not None and "purpose" in lc.columns:
        ...
        fig = go.Figure(go.Heatmap(
            z=heat_norm.values, x=heat_cols, y=heat_data.index.tolist(),
            ...
            showscale=True,
            colorbar=dict(tickfont=dict(color=TEXT_C), thickness=14, len=0.85,
                          title=dict(text="Normalised", font=dict(color=TEXT_C)),
    ...
    with col3:
        ...
        for tier, color in TIER_COLOR.items():
            ...
            if mask.sum() > 0:
                fig.add_trace(go.Scatter(
                    x=sample[mask]["credit_score"], y=sample[mask]["fraud_score"],
    ...
    with col4:
        ...
        dcols = [c for c in ["unified_score","credit_score","fraud_score","risk_tier"] if c in filtered.columns]
        st.dataframe(filtered.nlargest(20, "unified_score")[dcols].reset_index(drop=True),
...
# ═════════════════════════════════════════════════════════════
    ...
    <div class='page-subtitle'>Upload a CSV of customers — AI scores every row instantly and shows risk analytics</div>
    <div class='accent-line'></div>
    """, unsafe_allow_html=True)
    ...
    st.caption("Tip: Use outputs/data/unified_risk_scores.csv as a demo file!")
    ...
    if uploaded:
        ...
        if st.button("⚡ Run AI Risk Scoring on All Customers"):
            with st.spinner("AI is analysing all customers…"):
                ...
                for col in ib_final.select_dtypes(include=["object","category"]).columns:
                    le = LabelEncoder()
                    ib_final[col] = le.fit_transform(ib_final[col].astype(str))
                ...
                batch["risk_tier"]     = [tier_label(s) for s in us]
                batch["decision"]      = ["REJECT" if s >= 0.5 else "APPROVE" for s in cs]
            ...
            with col1:
                ...
                fig.add_annotation(text=f"{len(batch):,}<br>customers", x=0.5, y=0.5,
            ...
            with col2:
                st.markdown("<div class='card'>", unsafe_allow_html=True)
                st.markdown("<div class='section-label'>Approve vs Reject</div>", unsafe_allow_html=True)
                fig = go.Figure(go.Pie(
                    labels=["APPROVE","REJECT"], values=[approve, reject], hole=0.65,
                    marker=dict(colors=[TEAL, ROSE], line=dict(color="#080c14", width=4)),
                    textinfo="label+percent", textfont=dict(size=13, color="#f1f5f9"),
                    hovertemplate="<b>%{label}</b><br>%{value:,}<br>%{percent}<extra></extra>",
                ))
                fig.add_annotation(text=f"{approve:,}<br>approved", x=0.5, y=0.5,
                                   showarrow=False, font=dict(size=16, color="#f1f5f9"))
            ...
            with col3:
                st.markdown("<div class='card'>", unsafe_allow_html=True)
                st.markdown("<div class='section-label'>Credit Score Distribution</div>", unsafe_allow_html=True)
                fig = go.Figure()
                for t_val, color, name in [("LOW",TEAL,"Low Risk"),("MEDIUM",AMBER,"Medium Risk"),("HIGH",ROSE,"High Risk")]:
                    mask = batch["risk_tier"] == t_val
                    if mask.sum() > 0:
                        fig.add_trace(go.Histogram(
                            x=batch[mask]["credit_score"], name=name,
                            marker_color=color, opacity=0.75, nbinsx=30,
                            hovertemplate=f"<b>{name}</b><br>Score: %{{x:.3f}}<br>Count: %{{y}}<extra></extra>",
                        ))
            ...
            with col4:
                st.markdown("<div class='card'>", unsafe_allow_html=True)
                st.markdown("<div class='section-label'>Credit Score vs Fraud Score</div>", unsafe_allow_html=True)
                samp = batch.sample(min(2000, len(batch)), random_state=42)
                fig = go.Figure()
                for t_val, color in TIER_COLOR.items():
                    mask = samp["risk_tier"] == t_val
                    if mask.sum() > 0:
                        fig.add_trace(go.Scatter(
                            x=samp[mask]["credit_score"], y=samp[mask]["fraud_score"],
                            mode="markers", name=t_val,
                            marker=dict(color=color, size=4, opacity=0.6, line=dict(width=0)),
                            hovertemplate=f"<b>{t_val}</b><br>Credit: %{{x:.3f}}<br>Fraud: %{{y:.3f}}<extra></extra>",
                        ))
                apply_theme(fig, "Credit vs Fraud Score", height=320)
            ...
            with col5:
                st.markdown("<div class='card'>", unsafe_allow_html=True)
                st.markdown("<div class='section-label'>Unified Risk Score Range</div>", unsafe_allow_html=True)
                fig = go.Figure(go.Histogram(
                    x=batch["unified_score"], nbinsx=40,
                    marker=dict(color=batch["unified_score"],
                ...
                apply_theme(fig, "Unified Risk Score Distribution", height=300)
                fig.add_vline(x=0.30, line_color=TEAL, line_dash="dash", line_width=1.5,
                              annotation_text="Low/Med", annotation_font_color=TEAL)
                fig.add_vline(x=0.60, line_color=ROSE, line_dash="dash", line_width=1.5,
                              annotation_text="Med/High", annotation_font_color=ROSE)
            ...
            with col6:
                st.markdown("<div class='card'>", unsafe_allow_html=True)
                st.markdown("<div class='section-label'>Risk Tier Count</div>", unsafe_allow_html=True)
                tier_order = [t for t in ["LOW","MEDIUM","HIGH"] if t in tc.index]
                fig = go.Figure(go.Bar(
                    x=tier_order, y=[tc.get(t,0) for t in tier_order],
                    marker=dict(color=[TIER_COLOR[t] for t in tier_order],
                                line=dict(color="rgba(255,255,255,0.05)", width=1)),
                    text=[f"{tc.get(t,0):,}" for t in tier_order],
            ...
            # Top 10 riskiest
            st.markdown("<div class='card'>", unsafe_allow_html=True)
            st.markdown("<div class='section-label'>Top 10 Riskiest Customers — Immediate Attention Required</div>",
                        unsafe_allow_html=True)
            top10_cols = [c for c in ["unified_score","credit_score","fraud_score","risk_tier",
                                       "decision","loan_amnt","int_rate","dti","annual_inc"]
                          if c in batch.columns]
            top10 = batch.nlargest(10, "unified_score")[top10_cols].reset_index(drop=True)
            top10.index += 1
