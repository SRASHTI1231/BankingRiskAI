...
# ─────────────────────────────────────────────────────────────
    ...
    <div class='page-subtitle'>
        Upload a customer dataset to score the entire portfolio and export results for Power BI
    </div>
    <div class='accent-line'></div>
    """, unsafe_allow_html=True)
    ...
    st.code("loan_amnt · int_rate · dti · revol_util · annual_inc · total_acc · open_acc · pub_rec",
            language=None)
    uploaded = st.file_uploader("Upload customer file (.csv)", type=["csv"])
    st.markdown("</div>",unsafe_allow_html=True)

    if uploaded:
        batch = pd.read_csv(uploaded)
        st.success(f"{len(batch):,} customer records loaded successfully.")
        ...
        if st.button("Execute Batch Risk Scoring"):
            with st.spinner("Processing all customer records..."):
                ...
                if "dti"      in batch.columns: ib["high_dti_flag"] = (batch["dti"]>30).astype(int)
                if "int_rate" in batch.columns: ib["high_int_flag"] = (batch["int_rate"]>18).astype(int)

                cs = xgb.predict_proba(ib[feat_cols])[:,1]
                sc = scaler.transform(ib[feat_cols])
                ir = iso.decision_function(sc)
                fs = np.clip(1-(ir-risk_cfg["iso_score_min"])/(risk_cfg["iso_score_max"]-risk_cfg["iso_score_min"]+1e-9),0,1)
                us = risk_cfg["credit_weight"]*cs + risk_cfg["fraud_weight"]*fs

                def tier(s): return "HIGH" if s>=0.6 else "MEDIUM" if s>=0.3 else "LOW"
            ...
            st.success(f"Scoring complete — {len(batch):,} customers processed.")

            tc = batch["risk_tier"].value_counts()
            c1,c2,c3 = st.columns(3)
            c1.metric("High Risk",   int(tc.get("HIGH",0)))
            c2.metric("Medium Risk", int(tc.get("MEDIUM",0)))
            c3.metric("Low Risk",    int(tc.get("LOW",0)))

            # Results donut
            st.markdown("<div class='card'>",unsafe_allow_html=True)
            col_d, col_t = st.columns(2)
            with col_d:
                st.markdown("<div class='section-label'>Batch Results Distribution</div>",unsafe_allow_html=True)
                fig = go.Figure(go.Pie(
            ...
            with col_t:
                st.markdown("<div class='section-label'>Scored Records</div>",unsafe_allow_html=True)
                st.dataframe(batch.head(15), use_container_width=True, height=280)
            st.markdown("</div>",unsafe_allow_html=True)

            csv_out = batch.to_csv(index=False).encode("utf-8")
            st.download_button(
                label     = "Download Scored Results — CSV for Power BI",
                data      = csv_out,
